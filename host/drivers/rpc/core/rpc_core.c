...
static int process_rpc_rx_msg(Rpc * proto_msg, rpc_rx_ind_t rpc_rx_func)
{
	...
	/* HyperFi M2.9 debug probe removed 2026-04-24 post-M2.9 verification.
	 * These printf()s in the rpc_rx_thread hot path were saturating the
	 * SDIO double-buffer at 74 fps and causing "task still writing Rx data
	 * to queue!" warnings. Keep the code comment as a reminder — the probe
	 * is still useful if we see CSI msg routing regressions in M3. */

	/* Note: free proto_msg at the end of processing*/

	/* 2. Check if it is event msg */
	if (proto_msg->msg_type == RPC_TYPE__Event) {
		/* HyperFi ADR-022 patch: always pass through CustomRpc (788). */
		if (CALLBACK_AVAILABLE ==
				is_event_callback_registered(proto_msg->msg_id)
				|| proto_msg->msg_id == RPC_ID__Event_CustomRpc) {
			...
			/* Normal event path: call_event_callback invokes rpc_event_callback
			 * which CLEANUP_RPCs app_event itself.
			 *
			 * HyperFi M2.9 memory leak fix (2026-04-24): for Event_CustomRpc
			 * (788), rpc_parse_evt already dispatches to hf_custom_callbacks[]
			 * directly (see rpc_evt.c case RPC_ID__Event_CustomRpc) — those
			 * user callbacks do NOT CLEANUP_APP_MSG, so we must free here to
			 * match the ownership contract. Custom RPC is NOT re-dispatched
			 * via call_event_callback (rpc_evt_cb_table slot for 788 is NULL
			 * by design).
			 *
			 * At 183 fps CSI, missing this free leaked ~200 B/frame → heap
			 * exhausted in ~60 s. Adding an unconditional HOSTED_FREE after
			 * call_event_callback would be a double-free for the Non-Custom
			 * paths (rpc_event_callback's CLEANUP_RPC already ran). */
			if (proto_msg->msg_id == RPC_ID__Event_CustomRpc) {
				HOSTED_FREE(app_event);
			} else {
				call_event_callback(app_event);
				/* app_event freed inside rpc_event_callback's CLEANUP_RPC */
			}
		} else {
			goto free_buffers;
		}
...
static void rpc_rx_thread(void const *arg)
{
	...
	/* HyperFi M2.9 debug: rpc_rx_thread startup banner removed post-verification */
