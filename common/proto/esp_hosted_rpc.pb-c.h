...
typedef enum _RpcId {
  ...
  /*
   * HyperFi backport from v2.12.6 (ADR-022) for Custom RPC receive path.
   * Number 788 is fixed by upstream ID allocation and must match slave v2.12.6. 
   */
...
/*
 * HyperFi backport from v2.12.6 (ADR-022) — Custom RPC event payload.
 * Matches the upstream Rpc_Event_CustomRpc message definition verbatim.
 * Only the Event direction is ported (slave → host); Req/Resp (host → slave)
 * are intentionally omitted since HyperFi's CSI pipeline is slave-push only. 
 */
...
struct  Rpc
{
  ...
  union {
    ...
    /*
     * HyperFi backport from v2.12.6 (ADR-022) — Custom RPC event receive 
     */
